...
function VerifyEmailPage() {
	...
	const handleSubmit = async (formData: any) => {
		...
		try {
			...
			if (result === null) {
				setSuccessMessage('Yêu cầu xác thực email đã được gửi thành công. Vui lòng kiểm tra hộp thư đến của bạn.');
			}

			setSubmitDisabled(true);
			setCountdown(delayTime / 1000);
		} catch (error) {
			setErrorMessage('Đã xảy ra lỗi khi gửi yêu cầu xác thực email. Vui lòng thử lại sau.');
		} finally {
			setIsLoading(false);
		}
	};
